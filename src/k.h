#ifndef K__H
#define K__H

#include "k.core/k.h"

#define st(t,z) ((K)(t)<<48|((K)(z)&b(48)))
#define set_sx(x,s) (((x) & ~ (b(56) & ~b(48))) | ((K)(s) << 48))

#define VST(x) (!s(x) || s(x) & 0x80)
#define ISF(x) ((x)<256 || (VST(x) && s(x)&0x40))

#ifdef _WIN32
#define MAXR 100
#else
#define MAXR 1000
#endif
#define EMAX 14

/* Eval recursion ceiling = rows of the A0 value-stack / params pools (p.c).
 * maxr is clamped to this so the depth COUNTER can't overrun those arrays; the
 * real C-stack limit is enforced separately by stack_low() below. */
#define EVALDEPTH 4096

/* RSP-based stack guard.  Recursion is bounded by actual bytes-of-stack-remaining
 * rather than a fixed per-level count, so trivial recursion runs deep while a
 * heavy adverb-nested level (~10KB/level) stops safely well before overflow.
 * The stack grows down; stack_floor is the low-address danger threshold and
 * &local approximates the current SP (plain ISO C -- no builtins/attributes).
 * Under ASAN &local points into the shadow/fake stack and is meaningless, so the
 * guard is disabled there and the maxr count-cap governs instead. */
/* Two floors: eval stops at the higher stack_floor, reserving headroom BELOW it
 * so that error reporting / result printing (kprint_) -- which runs while the
 * eval stack may still be near full, e.g. echoing a deep result at the moment
 * the depth guard fired -- has room to work and uses the lower stack_floor_print.
 * Without this, eval erroring exactly at its floor leaves kprint_ at the floor
 * too, so it truncates ("...") spuriously. */
extern uintptr_t stack_floor;        /* eval guard (more reserve) */
extern uintptr_t stack_floor_print;  /* kprint_ guard (less reserve, deeper) */
extern uintptr_t stack_floor_sub;    /* error-subconsole eval guard (between the two) */
extern int ecount;                   /* >0 while inside a stack-error subconsole (repl.c) */
void stack_guard_init(void);
#if defined(ASAN_ENABLED) || defined(__EMSCRIPTEN__)
/* WASM: &local is a linear-memory address, not a real SP, and the OS stack-bounds
 * query in stack_guard_init() has no Emscripten branch -- disable the RSP guard and
 * let the maxr count-cap (lowered in kinit) govern depth instead, as under ASAN. */
static inline int stack_low(void) { return 0; }
static inline int stack_print_low(void) { return 0; }
#else
/* A stack-error subconsole opens IN PLACE (the recursion that tripped the guard is
 * still live on the C stack; only \ unwinds it), so at ecount>0 eval would re-trip
 * at stack_floor immediately.  Use the deeper stack_floor_sub there so \\ / simple
 * commands run, while keeping a safe reserve below it. */
static inline int stack_low(void) { char probe; return (uintptr_t)&probe <= (ecount?stack_floor_sub:stack_floor); }
static inline int stack_print_low(void) { char probe; return (uintptr_t)&probe <= stack_floor_print; }
#endif

extern i32 maxr;
extern char *E[EMAX];
extern i32 zeroclamp;
extern char *R_NUL,*R_DRAW,*R_DOT,*R_VS,*R_SV,*R_ATAN2,*R_DIV,*R_AND,*R_OR,*R_SHIFT,*R_ROT,*R_XOR,*R_HYPOT,*R_ENCRYPT,*R_DECRYPT,*R_SETENV,*R_RENAME,*R_SQR,*R_ABS,*R_SLEEP,*R_IC,*R_CI,*R_DJ,*R_JD,*R_LT,*R_LOG,*R_EXP,*R_SQRT,*R_FLOOR,*R_CEIL,*R_SIN,*R_COS,*R_TAN,*R_ASIN,*R_ACOS,*R_ATAN,*R_AT,*R_SS,*R_SM,*R_LSQ,*R_SINH,*R_COSH,*R_TANH,*R_ERF,*R_ERFC,*R_GAMMA,*R_LGAMMA,*R_RINT,*R_TRUNC,*R_NOT,*R_KV,*R_VK,*R_VAL,*R_BD,*R_DB,*R_HB,*R_BH,*R_ZB,*R_BZ,*R_MD5,*R_SHA1,*R_SHA2,*R_GETENV,*R_SVD,*R_LU,*R_QR,*R_LDU,*R_RREF,*R_DET,*R_MAG,*R_PRIME,*R_FACTOR,*R_GCD,*R_LCM,*R_MODINV,*R_EXIT,*R_DEL,*R_DO,*R_WHILE,*R_IF,*R_IN,*R_DVL,*R_LIN,*R_DV,*R_DI,*R_GTIME,*R_LTIME,*R_TL,*R_MUL,*R_INV,*R_CHOOSE,*R_ROUND,*R_SSR,*R_EP,*R_TIMER,*R_WATCH;

K kcpcb(K x);
static inline K kcp2(K x) { return s(x)?kcpcb(x):k_(x); }
static inline K kerror(char *e) { return t(4,st(0x84,sp(e))); }

void kinit(void);
const char* kprint_(K x, char *s, char *e, char *s0);
void kprint(K x, char *s, char *e, char *s0);
i32 kreserved(char *p);
K val(K x);
K kamendi3(K d, K i, K f);
K kamendi4(K d, K i, K f, K y);
K kamend3(K d, K i, K f);
K kamend4(K d, K i, K f, K y);
K kslide(K f, K a, K x, char *av);
void kdump(i32 l);
void kdp(K x);

#endif /* K__H */
