...
const char* kprint_(K x, char *s, char *e, char *s0) {
  ...
  while(sp>0) {
    ...
    case 0xc0:;
      ...
      break;
    case 0xda: { /* (f;av) modified-verb wrapper, replaces 0xc1/0xc2 */
      K *pw=px(x); K wf=pw[0]; K wav=pw[1];
      char *avp=(T(wav)==-3 && n(wav)>0) ? (char*)px(wav) : "";
      if(0xc3==s(wf)) {
        /* Pass 2b-step-4: print inner lambda then suffix the av.
           0xc4 retired in Pass 4 -- the inner of a 0xda is now
           always 0xc3. */
        kprint_(wf,s,"",s0);
        mprintf("%s%s",avp,e);
        break;
      ...
      if(0xd9==s(wf)) {
        /* Issue #2 Pass 9: option-B display for adverbed projection.
           The structure 0xda(0xd9(0xd9(... f ...), args_outer), av)
           encodes args bound from inner to outer with the av string
           lumped on top.  Display as `f[args0]<av0>[args1]<av1>...`
           by walking the d9 chain and interleaving one av char per
           bracket boundary so `f[1]'[2]` prints as `f[1]'[2]`
           (preserving the input shape) rather than the canonical
           `f[1][2]'`.  Leftover av characters trail the outermost
           bracket. */
        K stack9[64]; int nstack=0;
        K base=wf;
        while(0xd9==s(base) && nstack<64) {
          K *pd=px(base);
          stack9[nstack++]=pd[1];
          base=pd[0];
        }
        kprint_(base,s,"",s0);
        u64 navp_n=strlen(avp);
        for(int li=nstack-1; li>=0; --li) {
          mprintf("[");
          {
            K g2=stack9[li]; K *pg2=px(g2);
            i(n(g2),kprint_(pg2[i],"","",s0);if(i<n(g2)-1) mprintf(";"))
          }
          mprintf("]");
          int slot=(nstack-1)-li; /* 0 = innermost args boundary */
          if((u64)slot<navp_n) {
            char ac=avp[slot];
            mprintf("%c",ac);
          }
        }
        if(navp_n>(u64)nstack) mprintf("%s",avp+nstack);
        mprintf("%s",e);
        break;
      }
      if(0xda==s(wf) || 0xc5==s(wf)) {
        /* Issue #2 Pass 5 step 3: 0xda(0xd9, av) projection-
           with-each form generated by fapply NEST+av-lift.
           Print inner shape then suffix av.  (Pass 9: the d9
           case above peels its own chain; this branch still
           covers nested 0xda/0xc5 inner. */
        kprint_(wf,s,"",s0);
        mprintf("%s%s",avp,e);
        break;
      }
    ...
    case 0xc3:
      /* Issue #2 Pass 6 cleanup: pf[3] is valence now, not av. */
      px=px(x);
      mprintf("%s%s%s",s,(char*)px(px[0]),e);
      break;
    ...
    case 0:
      ...
      case  0:
        if(nx==1) {
          ...
          i(nx,t=T(px[i]);if(!s(px[i])&&t==-3)++ncv); if(nx==ncv) a=0; /* ("asdf";"asdf") */
...
K val(K x) {
  ...
  case 0xda: { /* (f;av) modified-verb wrapper, Issue #2 Pass 5
                  step 2.  Compute valence by peeling av left-to-
                  right over the inner's valence:
                    "'" (each)        preserves valence,
  ...
  case 0xd9: { /* (f;args) projection wrapper, Issue #2 Pass 3a.
                  val((f;args)) = max(val(f) - nb, ni, 1) where:
                    nb = count of bound (non-inull) entries in args,
                    ni = count of inull entries in args.
                  The ni floor matters for primitives where val(f)
                  understates the slot count (e.g. val(@K)=2 but
                  @[;;] really has 3 holes -- amend's 3-/4-arg
                  forms). The max(..,1) clamp matches builtin/0xc8
                  single-slot behavior. */
    K *pw=px(x); K wargs=pw[1];
    K vf=val(pw[0]); if(E(vf)) { r=vf; break; }
    i32 vn=ik(vf); _k(vf);
    K *pw1=px(wargs);
    i32 nb=0, ni=0;
    i(n(wargs),if(pw1[i]==inull)++ni;else++nb)
    i32 rem=vn-nb;
    if(rem<ni) rem=ni;
    if(rem<1) rem=1;
    r=t(1,rem);
...
static K kamendi3d(K d, K i, K f) {
  ...
  case 0: {
    ...
    while(sp) {
      ...
      while(pf->j<n(i_)) {
        K k=piu[pf->j++];
        if(s(k)) { e=KERR_TYPE; goto cleanup; }
        if(T(k)==0) {
          if(sp==sm) { stack=xrealloc(stack,sizeof(sf)*(sm*=2)); }
          stack[sp++]=(sf){k,0};
          goto continue_outer;
...
static K kamendi4d(K d, K i, K f, K y) {
  ...
  case  6:
    ...
    case 15:
      if(b) i(n(v),r=amend_fe(k_(f),k_(pv[i]),k_(y),0);EC(r);_k(pv[i]);pv[i]=r)
      else i(n(v),{K cpv=kcp(y);EC(cpv);_k(pv[i]);pv[i]=cpv;})
      break;
  ...
  case  0: {
    ...
    while(sp) {
      ...
      K y_=pf->y;
      K *piu=px(i_);
      i8 Ty_=T(y_); if(s(y_)) { if(!VST(y_)) { e=KERR_TYPE; goto cleanup; } Ty_=15; }
      switch(Ty_) {
      case -1: pyi=px(y_);
      ...
      case 1: case 2: case 3: case 4: case 6: case 8: case 9: case 15:
        while(pf->j<n(i_)) {
          ...
          if(T(i2)==0) {
            ...
            d=kamendi4d(d,k_(i2),k_(f),k_(y_));
            if(E(d)) { e=d; goto cleanup; }
          }
        }
        break;
...
#define ASSIGN_LOOP(N, IDXEXPR, LHS, RHS, RAW, EXPECTED, KNPACK) \
  for(u64 j=0;j<(N);++j) { \
    ...
    if(pdu) { \
      ...
      if(!s(r) && T(r)==EXPECTED) { RAW[idx]=KNPACK; _k(r); } \
  ...
  if(ti==8||ti==-8) { /* long index into (possibly >2^31) vector */
    if(ti==-8) { /* long index vector: amend each position (reuses ti==8 base) */
      ...
      if(td&&abs(td)==abs(Ty)) {
        ...
        else { K cpv=kcp2(y); EC(cpv); _k(pdu[idx]); pdu[idx]=cpv; }
      }
    }
  }
    ...
    case -2: pdf=px(d);
      ...
      case  1:
        ...
        if(!s(r) && T(r)==2) { pdf[ik(i)]=fk(r); _k(r); }
        else {
          t=kmix(d); EC(t);
    ...
    case -8: pdj=px(d);
      ...
      case  1:
        ...
        if(!s(r) && T(r)==8) { pdj[ik(i)]=jk(r); _k(r); }
        else {
          t=kmix(d); EC(t);
          _k(d); d=t; pdu=px(d); _k(pdu[ik(i)]); pdu[ik(i)]=r;
        }
    ...
    case -9: pde=px(d);
      ...
      case  1:
        ...
        if(!s(r) && T(r)==9) { pde[ik(i)]=ek(r); _k(r); }
        else {
          t=kmix(d); EC(t);
          _k(d); d=t; pdu=px(d); _k(pdu[ik(i)]); pdu[ik(i)]=r;
        }
    ...
    case  1:
      ...
      else { K cpv=kcp2(y); EC(cpv); _k(pdu[ik(i)]); pdu[ik(i)]=cpv; }
      break;
    case -1:
      switch(Ty) {
      case  1: case 2: case 3: case 4: case 6: case 8: case 9:
      ...
      case 15:
        if(b) i(n(i),r=amend_fe(k_(f),k_(pdu[pii[i]]),k_(y),0); EC(r); _k(pdu[pii[i]]); pdu[pii[i]]=r)
        else i(n(i),{K cpv=kcp(y);EC(cpv);_k(pdu[pii[i]]);pdu[pii[i]]=cpv;})
        break;
      case -1: pyi=px(y);
      ...
      case  0: pyu=px(y);
        ...
        else i(n(i),{K cpv=kcp2(pyu[i]);EC(cpv);_k(pdu[pii[i]]);pdu[pii[i]]=cpv;})
        break;
      } break;
    case  6:
      switch(Ty) {
      ...
      case 15:
        if(b) i(n(d),r=amend_fe(k_(f),k_(pdu[i]),k_(y),0); EC(r); _k(pdu[i]); pdu[i]=r)
        else i(n(d),{K cpv=kcp(y);EC(cpv);_k(pdu[i]);pdu[i]=cpv;})
        break;
      case -1: pyi=px(y);
        if(n(d)!=n(y)) { e=KERR_LENGTH; goto cleanup; }
      ...
      case  0: pyu=px(y);
        ...
        else i(n(d),{K cpv=kcp2(pyu[i]);EC(cpv);_k(pdu[i]);pdu[i]=cpv;})
        break;
      } break;
    case  0: {
      stack=xmalloc(sizeof(sf)*sm);
        ...
        case 1: case 2: case 3: case 4: case 6: case 8: case 9: case 15:
          while(pf->j<n(i_)) {
            ...
            if(T(i2)==0) {
              ...
              d=kamendi4v_(d,k_(i2),k_(f),k_(y_));
              if(E(d)) { e=d; goto cleanup; }
            }
          }
          break;
...
#define PROPAGATE_RESULT(x) do { \
  ...
  while(sp) {
    ...
    K r_ = pf->r;
    K ri_ = pf->ri;
    if(s(i_)) { e=KERR_TYPE; goto cleanup; }
    ...
    case 0:
      if(0x80 == s(d_)) {
        ...
        case  0:
          ...
          _k(i2); i2=t;
          if(s(i0)) { _k(i0); _k(i2); e=KERR_TYPE; goto cleanup; }
          switch(T(i0)) {
          case 4:
...
K kslide(K f, K a, K x, char *av) {
  ...
  u32 n;
  u64 s,v=2,m;  /* nx and an i64 stride may both exceed 2^32 */
  i64 stride;
  K ff=f;
  i8 Tx;
  ...
  Tx=tx; if(s(x)) { if(!VST(x)) { e=KERR_TYPE; goto cleanup; } Tx=15; }
  if(!intatom(a)) { e=KERR_TYPE; goto cleanup; }
  stride=ta==8?jk(a):ik(a);
  if((ta==1 && (stride==INT32_MAX || stride==INT32_MIN || stride==INT32_MIN+1))
     || (ta==8 && (stride==J_INF || stride==J_NULL || stride==J_NINF))) {
    e=KERR_DOMAIN; goto cleanup;
  }
  if(!stride) { e=KERR_TYPE; goto cleanup; }
  ...
  if(Tx>0) {
    ...
    else if(stride<0) i(m, prk[i]=fe(k_(ff),k_(pxm[i*s+1]),k_(pxm[i*s]),av); EC(prk[i]))
    else if(stride>0) i(m, prk[i]=fe(k_(ff),k_(pxm[i*s]),k_(pxm[i*s+1]),av); EC(prk[i]))
  }
