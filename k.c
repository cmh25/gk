...
char* kprint_(K *p, char *s, unsigned int plevel, int nl) {
  ...
  case 11:
    oc+=sprintf(o+oc,"[");
    if(p->c) {
      DO(p->c-1,xfree(kprint_(v0(p)[i],"",plevel,0));oc+=sprintf(o+oc,";"))
      xfree(kprint_(v0(p)[p->c-1],"",plevel,0));
    }
    else xfree(kprint_(p,"",plevel,0));
    oc+=sprintf(o+oc,"]");
    break;
  case  0:
    if(p->c == 1) {
      oc+=sprintf(o+oc,"%s,",s);
      xfree(kprint_(v0(p)[0],"",plevel+1,0));
      break;
    }
